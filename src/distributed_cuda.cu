...
__global__ void pegasos_per_thread(int num_samples, int num_features, double * W, double * X, double * Y, double lambda, int num_iters, double * random_arr,  int k) {
    ...
    for(int sample=0; sample<n_samples_per_thread; sample++)
    {
        double lr = 1.0 / (lambda * (sample+1));
        double pred_output = 0;
        for (int i=0; i<num_features; i++)
            pred_output += W[index * num_features + i] * X[(n_samples_per_thread * index + sample) * num_features + i];
        if (Y[n_samples_per_thread * index + sample] * pred_output >= 1.0) {
            for (int i=0; i<num_features; i++)
                W[index * num_features + i] = (1.0 - lr * lambda) * W[index * num_features + i];
        } else {
            for (int i=0; i<num_features; i++)
                W[index * num_features + i] = (1.0 - lr * lambda) * W[index * num_features + i] + (lr * Y[n_samples_per_thread * index + sample]) * X[(n_samples_per_thread * index + sample) * num_features + i];
        }
    }
}
...
int main() {
    ...
    for (int i=0;i<n_features;i++)  final_W[i] /= k;

    // cout << "\nFinalW\n";
    // for (int i=0;i<n_features;i++)  {cout << final_W[i] << " ";}
